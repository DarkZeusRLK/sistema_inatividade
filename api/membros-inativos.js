...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // Calculamos o timestamp limite (8 dias atrás para segurança)
    ...
    const buscarMensagensCanal = async (channelId) => {
      ...
      // Mantivemos o loop de 50 como segurança máxima, mas o break abaixo é quem manda
      for (let i = 0; i < 50; i++) {
        ...
        try {
          ...
          if (!res.ok) break; // Se der erro (ex: 429 ou 403), para este canal
          ...
          // LOGICA INTELIGENTE:
          // Pega a data da última mensagem desse lote
          const lastMsgDate = new Date(
            batch[batch.length - 1].timestamp
          ).getTime();

          // Se a última mensagem já é mais velha que o nosso limite (8 dias),
          // não precisamos buscar mais nada para trás. Economiza API e evita Timeout.
    ...
    if (FERIAS_CHANNEL_ID) {
      try {
        ...
        // Férias buscamos menos histórico (ex: 5 páginas) pois é um canal menos movimentado
        for (let i = 0; i < 5; i++) {
